...
- endpoint degrades to 200 when LLM raises (real patch, not inert override)
- cert verdict wins even when an active LLM produces a different verdict_line
"""
...
def client_and_mocks():
    ...
    Note: get_llm_provider is NOT overridden via dependency_overrides — that override is inert
    because the endpoint calls it as a direct function, not via Depends. Individual tests that need
    to control the LLM use unittest.mock.patch("src.api_v2.get_llm_provider", ...).
    """
...
def test_briefing_verdict_comes_from_certificate_not_llm(client_and_mocks):
    """Cert verdict ('apto') must win; the LLM returns a different verdict_line ('NO-APTO-LLM')
    but the response must expose the cert value, not the LLM value."""
    client, mocks = client_and_mocks
    mocks.assurance.get_run_assurance_data.return_value = _run_data()
    mocks.cert.get_certificate.return_value = {"verdict": "apto", "risk_score": 0.1,
                                                "canonical_json": "{}"}
    mocks.actions.list_actions_for_run.return_value = []

    class _LLMWithDifferentVerdict:
        def complete(self, prompt: str) -> str:
            return (
                '{"summary":"s","verdict_line":"NO-APTO-LLM","highlights":["h1"],'
                '"recommendation":"r","citations":["cert"]}'
            )

    with patch("src.api_v2.get_llm_provider", return_value=_LLMWithDifferentVerdict()):
        r = client.get("/v2/runs/r1/briefing")

    assert r.status_code == 200
    body = r.json()
    assert body["verdict"] == "apto"          # from the certificate
    assert body["verdict"] != "NO-APTO-LLM"  # not from the LLM
...
def test_briefing_response_schema(client_and_mocks):
    ...
    client, mocks = client_and_mocks
    mocks.assurance.get_run_assurance_data.return_value = _run_data()
    mocks.cert.get_certificate.return_value = {"verdict": "no-apto", "risk_score": 5,
                                               "canonical_json": "{}"}
    mocks.actions.list_actions_for_run.return_value = [
...
def test_briefing_degrades_when_llm_down(client_and_mocks):
    """When get_llm_provider raises (real patch, not inert dependency_override) the endpoint
    still returns 200 with a non-empty degraded summary and the cert's verdict."""
    client, mocks = client_and_mocks
    mocks.assurance.get_run_assurance_data.return_value = _run_data()
    mocks.cert.get_certificate.return_value = {"verdict": "apto", "risk_score": 0,
                                               "canonical_json": "{}"}
    mocks.actions.list_actions_for_run.return_value = []

    with patch("src.api_v2.get_llm_provider", side_effect=RuntimeError("llm down")):
        r = client.get("/v2/runs/r1/briefing")
