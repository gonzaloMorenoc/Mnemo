...
def test_maps_rca_to_leccion_fields():
    family = {"id": "fam1", "title": "Timeout en checkout", "label": "checkout"}
    rca = {
    ...
    # tags = etiqueta de familia + proyectos (NO las citations, que son ids)
    assert p["tags"] == ["checkout", "web", "mobile"]
...
def test_tags_dedupe_label_and_projects():
    family = {"id": "f", "title": "T", "label": "web"}
    rca = {"root_cause": "c", "how_to_fix": "h"}
    p = rca_to_proposal(family, rca, projects=["web", "api"])
