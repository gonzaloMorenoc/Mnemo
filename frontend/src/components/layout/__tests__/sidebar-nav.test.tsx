...
describe("SidebarNav — secciones y navegación", () => {
  ...
  it("muestra los 15 enlaces con labels ES (incluye Continuidad, Guía, Verificar acta, Integraciones, Organización, Ajustes)", () => {
    (usePathname as ReturnType<typeof vi.fn>).mockReturnValue("/app");
    render(<SidebarNav />);
    ...
    expect(navLinks).toHaveLength(15);
    expect(screen.getByRole("link", { name: /^Continuidad$/i })).toBeInTheDocument();
    expect(screen.getByRole("link", { name: /^Guía$/i })).toBeInTheDocument();
